...
class UUID{
	...
	/**
	 * Creates a UUID from an hexadecimal representation
	 */
	...
	public function toString() : string{
		...
		if($this->version !== null){
			return substr($hex, 0, 8) . "-" . substr($hex, 8, 4) . "-" . hexdec((string) $this->version) . substr($hex, 13, 3) . "-8" . substr($hex, 17, 3) . "-" . substr($hex, 20, 12);
		}
