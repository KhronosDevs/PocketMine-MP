...
26. ✅ **Benchmark `measure_pipeline.php`** (off/gate/apply × 1000 entities): gate 0 mismatches but worker JSON transport is the scaling bottleneck (most results `lagged`); next step is the transport optimization
27. ⏳ **Next: Phase 9.2+** — faster snapshot transport (binary-packed / batched), then NetworkThread batching, async chunk gen, migration/load balancing (see docs/PLAN.md §5)