...
| 14.16 | ✅ | **Furnaces + smelting** — `SmeltingRegistry` (24 recipes / 16 fuels, legacy values), `FurnaceStore` per-block 3-slot state with TileEntitySnapshot round-trip, `FurnaceSystem` (fuel burn, 200-tick cook, lit 61↔62 flips), window-3 wire moves, break spill; **2 real persistence bugs fixed** (eviction saved chunks without tile snapshots; closure-bind + dirty-flag) — `tests/24` (5/27), `tests/17` +3 wire tests |
| 14.17 | ✅ | **Bows + arrows** — draw/release wire flow, `ArrowSystem` flight (drag + gravity via PhysicsSystem), block/entity stick, `ceil(speed/20*2)` damage + crits, shooter immunity window, 1200-tick despawn — `tests/25` (8/33) |
| 14.18 | ✅ | **Projectiles** — `ProjectileRegistry` single source (Arrow → network id 80), in-flight rendering (arrow on velocity vector), stuck arrows ride the victim and drop on death, shooter metadata attribution — `tests/25` |
| 14.19 | ✅ | **Terrain + biomes** — multi-octave value noise with 512/128/32-block feature cells, 8 biomes (temperature + humidity), biome-matched surface profiles, spawn plateau keeps origin land, deterministic trees/vegetation populate pass — `tests/26` (8 / 126,494 assertions) |
| 14.20 | ✅ | **Multi-world** — `WorldComponent` + `WorldRegistry` (per-world ChunkStore/WorldConfig/seed/StoragePort), Server API load/generate/unload, `/world` command, `switchWorld` wire flow (fresh chunk stream, explicit old-world entity despawn, per-world time/spawn), cross-world entity isolation — `tests/27` (6/32) |
| 14.20b | ✅ | **Infinite world streaming** — chunk queue re-queues on boundary cross so chunks stream continuously, bounded by a per-tick budget — wire test in `tests/17` |
| 10.5 | ✅ | **Real Anvil/McRegion storage** — clean `src/pocketmine/nbt/` codec (13 tags), `RegionStorageAdapter` (real 4KiB-sector MCA/MCR container, zlib big-endian NBT records), `AnvilStorageAdapter` (real vanilla `.mca`, negative coords), `McRegionStorageAdapter` (`.mcr` 128-height), `LevelProviderManager` auto-detect + LevelDB clear-error stub, real `level.dat` world meta — `tests/28` (9/100) |
| 14.21 | ✅ | **Real-client fixes** — `/tp` + respawn self-teleport eid 0 (protocol 84), world seed persistence (save falls through 0 → `ServerConfig::getSeed()`, `WorldConfig` synced on restore), returning-player safe-spawn guard (non-solid feet + head) — wire asserts in `tests/17`, boot-restore assert in `tests/10` |
| 14.22 | ✅ | **Weather cycle** — `WeatherSystem` (legacy calcWeather: weighted spell table, 6000-12000 tick spells, storm always clears), per-world state persisted in level.dat, transition LevelEventPackets (3001/3003 rain + 3002/3004 thunder) on login/change, lightning bolt (AddEntityPacket type 93) every 200 ticks during storms; `tests/29` (6/23) + `tests/17` wire test |
