...
namespace pocketmine {
    ...
    if (version_compare("8.2.0", PHP_VERSION) > 0) {
        echo "[CRITICAL] You must use PHP >= 8.2.0" . PHP_EOL;
    ...
    if ($errors > 0) {
        $logger->critical("Please install PHP 8.2 with the pmmpthread extension.");
        $logger->shutdown();
        $logger->join();
