	...
	public function onUpdate($currentTick){
		...
		if ($this->willMove(100)) {
			...
			if($this->isAlive()){
				...
				if($this->y > $this->highestY && $this->flyDirection !== null){
					$this->flyDirection->y = -0.5;
				}

				$inAir = !$this->isInsideOfSolid() && !$this->isInsideOfWater();
				if(!$inAir){
					$this->flyDirection = null;
				}
...
*/
				...
				if($this->onGround && $this->flyDirection instanceof Vector3){
		...
		return !$this->onGround || abs($this->motionX) > 0.00001 || abs($this->motionY) > 0.00001 || abs($this->motionZ) > 0.00001;
	}
