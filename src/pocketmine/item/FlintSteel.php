	...
	public function onActivate(Level $level, Player $player, Block $block, Block $target, $face, $fx, $fy, $fz){
		if($target->getId() === Block::OBSIDIAN && $player->getServer()->netherEnabled){//黑曜石 4*5最小 23*23最大
			...
			if($count_x >= 4 && $count_x <= 23){//4 23
				...
				if($count_y >= 5 && $count_y <= 23){//5 23
					$count_up = 0;//上面
					for($ux = $x_min; ($level->getBlock($this->temporalVector->setComponents($ux, $y_max, $tz))->getId() == Block::OBSIDIAN && $ux <= $x_max); $ux++){
						$count_up++;
					}
			...
			if($count_z >= 4 && $count_z <= 23){//4 23
				...
				if($count_y >= 5 && $count_y <= 23){//5 23
					$count_up = 0;//上面
					for($uz = $z_min; ($level->getBlock($this->temporalVector->setComponents($tx, $y_max, $uz))->getId() == Block::OBSIDIAN && $uz <= $z_max); $uz++){
						$count_up++;
					}
		...
		if($block->getId() === self::AIR && ($target instanceof Solid)){
			...
			if($block->getSide(Vector3::SIDE_DOWN)->isTopFacingSurfaceSolid() || $block->canNeighborBurn()){
