...
final class CraftingService {
    ...
    public function craft(EntityRef $playerRef, array $grid, int $gridSize = 3): ?ItemStack {
        ...
        $metadata = $player->get(\pocketmine\domain\component\MetadataComponent::class);
    ...
    private function findRecipe(array $grid, int $gridSize): ?array {
        // In a full implementation, this would look up recipes from a registry
        // For now, return some basic recipes
    ...
    private function hasIngredients(EntityRef $playerRef, array $grid, array $recipe): bool {
        $player = \pocketmine\domain\ecs\EntityRef::create($playerRef->getId(), $this->world)->getEntity();
