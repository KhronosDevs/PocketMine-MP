...
namespace pocketmine\plugin\api\permission;
...
final class PermissionManager {
    ...
    private function calculatePermissionDefault(Permission $permission): void {
        if ($permission->getDefault() === self::DEFAULT_OP || $permission->getDefault() === self::DEFAULT_TRUE) {
            $this->defaultPermsOp[$permission->getName()] = $permission;
        }

        if ($permission->getDefault() === self::DEFAULT_NOT_OP || $permission->getDefault() === self::DEFAULT_TRUE) {
            $this->defaultPerms[$permission->getName()] = $permission;
        }
    }

    public function hasPermission(EntityRef $player, string $permission): bool {
